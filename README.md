...
An implementation of a Hash Map storing string-value pairs. Features the expected CRUD operations, a hash function to hash strings, and dynamic enlarging to minimize collisions. 
...
Not all indices will be unique due to the finite array size, resulting collisions. Collisions are resolved by using a linked list at each array slot to store collided elements. This results in a worst-case runtime of O(n) for insertion, retrieval, and deletion. 
